...
- `author.community.*` fields are **PARTIALLY TRUSTED** - they are generated by the community, but community owners can manipulate them
...
- **Domain wallet addresses** (e.g., ENS): Verified that the `pkc-author-address` TXT record matches the publication's signing key
...
The risk scoring system primarily relies on `author.community` data for reputation signals, with mitigations for manipulation.

**Note on `author.community` trust limitations**: While `author.community.*` fields are generated by the community (not the user), a malicious community owner can still manipulate values like `postScore` and `replyScore`. The count-based karma approach mitigates this by giving each community exactly one vote regardless of karma magnitude. Additionally, an author's `author.community` data only exists after they sign and publish a comment to that community—this means attackers cannot create thousands of communities to manipulate _someone else's_ reputation, since the target must actively participate in each sub.

**Note on timestamp trust**: `author.community.firstCommentTimestamp` and `comment.timestamp` are **NOT TRUSTED** for account age calculation. A malicious community owner can fabricate `firstCommentTimestamp`, and since communities only validate that `comment.timestamp` is "recent", an owner can backdate their own comments. We cannot detect community ownership at the protocol level (author signer keys are separate from community keys). Additionally, engine's `receivedAt` is not trusted because it counts all submissions including those rejected by communities - a spammer could inflate their "account age" by repeatedly submitting spam that gets blocked. Therefore, account age is calculated **only from indexer's `fetchedAt`** (when the comment was found in community pages, indicating it was accepted).
...
- Karma aggregation (author's reputation across communities)
...
2. **Indexer tables** (`indexed_comments_ipfs`, `indexed_comments_update`, `modqueue_*`): Populated by the background indexer which crawls communities
...
| Karma                   | Per-community, use LATEST entry from either source       |
...
Evaluates how long the author has been known to our system, using **only indexer's `fetchedAt` timestamp** (when comments were found in community pages).

**SECURITY**: We do **NOT** trust:

- `author.community.firstCommentTimestamp` - can be fabricated by malicious community owners
- `comment.timestamp` - community owners can backdate their own comments (communities only validate timestamps are "recent", which owners can bypass)
...
We cannot detect community ownership at the protocol level (author signer keys are separate from community keys), so we cannot filter "owned" vs "external" communities. The only secure approach is to trust only the indexer's `fetchedAt` - which represents comments that were actually accepted and included in community pages.

**Tradeoff**: Account age only reflects comments that were accepted by communities (appeared in their pages). This means authors who only have rejected submissions will appear as "no history" - which is the correct behavior since those submissions weren't accepted.
...
Instead of using raw karma values (which can be manipulated by colluding communities), we count how many communities the author has positive vs negative karma in. Each community gets exactly **1 vote** regardless of karma magnitude.
...
- **Current community**: From `author.community` (TRUSTED) in the challenge request
- **Other communities**: Aggregated from combined engine + indexer data, using the **latest** entry per community
...
**Mitigation - Domain-only karma**: To prevent self-promotion attacks, only karma from **domain-addressed communities** is counted. IPNS addresses (like `12D3KooW...`) are free to create, making them vulnerable to sybil attacks. Domain addresses (like `example.eth`, `example.sol`, `example.com`) cost money to acquire, creating an economic barrier that makes mass community creation impractical. Additionally, the `/evaluate` endpoint only accepts requests from domain-addressed communities that can be resolved via pkc-js.
...
**Note**: Results from both engine and indexer tables are combined to detect cross-community spam patterns.
...
Link counts from both sources are **summed** to detect cross-community link spam campaigns.
...
**Note**: Community edits are not tracked for velocity as they are administrative actions.

**Replay attack protection**: The `/evaluate` endpoint rejects duplicate publications by checking `publication.signature.signature` before insertion. If the same publication is submitted multiple times, subsequent attempts return a 409 Conflict error. This prevents malicious communities from artificially inflating an author's velocity count.
...
Evaluates the author's ban history across all indexed communities, scaled by community breadth. Only **active** bans are counted (where `banExpiresAt >= current_time`); expired bans are ignored.

This factor uses data collected by the indexer, which:

- Subscribes to communities discovered via the `/evaluate` API or `author.previousCommentCid` chains
- Tracks `CommentUpdate.author.community.banExpiresAt` to detect bans
- Aggregates ban data across all indexed communities
...
The indexer tracks `community.modQueue.pendingApproval` entries and detects resolution by:
...
Evaluates what percentage of the author's comments have been removed, purged, or disapproved across all indexed communities, using a **weighted formula** that reflects the severity of each action.
...
- **Purge** (irreversible): Comment is permanently deleted. For replies, the comment disappears from parent's reply pages. For posts, the comment disappears from community pages and cannot be fetched via IPFS after 3 verification attempts. Purge cascades to all descendants.
...
**Rationale**: Authors whose content is frequently removed or purged are likely posting inappropriate content. Purge is weighted highest because it represents an irreversible decision by the community owner — a stronger signal than a reversible removal.
...
- Nonce is fetched on each wallet's declared chain (via `pkc.chainProviders[chainTicker]`)
...
2. **New Community Limitation**: For new communities, `author.community` data may be empty for all users initially.
