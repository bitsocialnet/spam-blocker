/**
 * Shared OAuth provider display configuration.
 * Used by all iframe generators that show OAuth buttons.
 */

import type { OAuthProvider } from "./types.js";

export const PROVIDER_CONFIG: Record<OAuthProvider, { name: string; color: string; hoverColor: string; icon: string }> = {
    github: {
        name: "GitHub",
        color: "#24292e",
        hoverColor: "#1a1e22",
...
/**
 * Generate OAuth button HTML for given providers.
 */
export function generateOAuthButtons({
    ...
    return providers
        .map((provider) => {
            ...
            const isCompleted = completedProvider === provider;

            if (isCompleted) {
                return `
                <div class="oauth-btn-completed">
                    <span class="icon">${config.icon}</span>
                    <span>Signed in with ${config.name}</span>
