...
test("normalizes Claude and Cursor edits, including paths with shell metacharacters", () => {
    assert.deepEqual(editedPaths({ hook_event_name: "PostToolUse", tool_name: "Write", tool_input: { file_path: "/repo/file.ts" } }), [
        "/repo/file.ts"
    ]);
    assert.deepEqual(editedPaths({ hook_event_name: "afterFileEdit", file_path: "src/a $(touch sentinel).tsx" }), [
...
test("read-only, failed, malformed, and irrelevant invocations do no work", (t) => {
    ...
    write("node_modules/prettier/bin/prettier.cjs", "{}");
    write("file.ts");
    const payloads = [
        undefined,
        null,
...
test("formats each existing edited file once through installed Prettier without package-manager bootstrap", (t) => {
    const { root, write } = fixture(t);
    write("node_modules/prettier/bin/prettier.cjs", "{}");
    ...
    formatEditedFiles({ tool_name: "apply_patch", tool_input: { command } }, root, (executable, args, options) => {
        ...
        assert.deepEqual(args, [path.join(root, "node_modules/prettier/bin/prettier.cjs"), "--write", first, second]);
