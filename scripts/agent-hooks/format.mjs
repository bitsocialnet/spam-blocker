...
export function formatEditedFiles(payload, root = repoRoot, run) {
    ...
    if (!files.length || !existsSync(path.join(root, "node_modules/prettier/bin/prettier.cjs"))) return;

    // Invoke only the installed formatter; a fresh checkout never installs dependencies.
    const execute = run ?? spawnSync;
    const result = execute(process.execPath, [path.join(root, "node_modules/prettier/bin/prettier.cjs"), "--write", ...files], {
        cwd: root,
        encoding: "utf8",
